...
Cobertura:
    - Construção de grafo vazio
    - Adição de vértices (válidos e duplicados)
    - Adição de arestas (válidas, com ids inexistentes, com peso negativo)
...
class TestGraphConstruction:
    """Testes de construção e estado inicial do grafo."""
...
class TestAddEdge:
    ...
    def test_directed_graph_has_one_directed_edge_per_add(self):
        ...
        graph.add_vertex(Vertex(id=1, label="B"))
        graph.add_edge(Edge(origin=0, destination=1, weight=10.0))
        # Em grafo dirigido, 0->1 não implica 1->0
    ...
    def test_undirected_graph_creates_reverse_edge(self):
        ...
        graph.add_vertex(Vertex(id=1, label="B"))
        graph.add_edge(Edge(origin=0, destination=1, weight=10.0))
        # Em grafo não-dirigido, ambos os sentidos devem existir
...
class TestNeighbors:
    """Testes de consulta de vizinhos."""
    ...
    def test_neighbors_returns_copy_not_reference(self):
        """Mutar o retorno de neighbors() não deve afetar o grafo."""
        graph = Graph()
        graph.add_vertex(Vertex(id=0, label="A"))
        graph.add_vertex(Vertex(id=1, label="B"))
        graph.add_edge(Edge(origin=0, destination=1, weight=10.0))
        ...
        # Estado interno do grafo não deve mudar
        assert graph.neighbors(0) == [(1, 10.0)]
