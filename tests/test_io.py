"""Testes unitários para JSONReader e JSONWriter."""
...
class TestJSONReader:
    ...
    def test_read_duplicated_vertex_id_raises(self, tmp_path):
        ...
        bad = {
            "vertices": [{"id": 0, "label": "A"}, {"id": 0, "label": "B"}],
            "edges": []
        }
    ...
    def test_read_negative_weight_raises(self, tmp_path):
        ...
        bad = {
            "vertices": [{"id": 0, "label": "A"}, {"id": 1, "label": "B"}],
...
class TestJSONWriter:
    ...
    def test_write_route_creates_file(self, tmp_path):
        ...
        JSONWriter.write_route(
            path=out, origin=0, destination=6,
            path_vertices=[0, 3, 4, 6], total_cost=260.0,
            algorithm="dijkstra", time_ms=1.234,
        )
        assert out.exists()
    ...
    def test_write_route_creates_parent_directories(self, tmp_path):
        ...
        JSONWriter.write_route(
            path=out, origin=0, destination=1,
            path_vertices=[0, 1], total_cost=10.0,
            algorithm="dijkstra", time_ms=0.5,
        )
        assert out.exists()
